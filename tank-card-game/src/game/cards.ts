import type { TankCard } from "./types";

const rawCards: TankCard[] = [
  {
    id: "t34_76",
    name: "T-34/76",
    nation: "ussr",
    class: "medium",
    rarity: "common",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "kv1",
    name: "КВ-1",
    nation: "ussr",
    class: "heavy",
    rarity: "uncommon",
    cost: 4,
    attack: 3,
    armor: 2,
    hp: 7,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "su76",
    name: "СУ-76",
    nation: "ussr",
    class: "td",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 0,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    onPlayEffects: { draw: 2 },
    abilityText:
      "",
  },
  {
    id: "ms_1_t18",
    name: "МС-1",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "t26_1931",
    name: "Т-26 1931",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "t26_1933",
    name: "Т-26 1933",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "t26_1938",
    name: "Т-26 1938",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "bt_2",
    name: "БТ-2",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    combatAbilities: { blitz: true },
    abilityText:
      "",
  },
  {
    id: "bt_5",
    name: "БТ-5",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    combatAbilities: { blitz: true },
    abilityText:
      "",
  },
  {
    id: "bt_7",
    name: "БТ-7",
    nation: "ussr",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 5,
    abilityText:
      "",
    combatAbilities: { blitz: true, raidDraw: 1 },
  },
  {
    id: "t37a",
    name: "Т-37А",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 5,
    abilityText:
      "",
    onPlayEffects: { draw: 1 },
  },
  {
    id: "t38",
    name: "Т-38",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 1,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    abilityText:
      "",
    onPlayEffects: { draw: 2 },
  },
  {
    id: "t40",
    name: "Т-40",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    abilityText:
      "",
    combatAbilities: { blitz: true },
  },
  {
    id: "t24",
    name: "Т-24",
    nation: "ussr",
    class: "medium",
    rarity: "common",
    cost: 3,
    attack: 1,
    armor: 1,
    hp: 7,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 2,
    abilityText:
      "",
  },
  {
    id: "t28",
    name: "Т-28",
    nation: "ussr",
    class: "medium",
    rarity: "uncommon",
    cost: 5,
    attack: 3,
    armor: 1,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 2,
    abilityText:
      "",
  },
  {
    id: "t35",
    name: "Т-35",
    nation: "ussr",
    class: "heavy",
    rarity: "rare",
    cost: 6,
    attack: 2,
    armor: 2,
    hp: 9,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "t46_1",
    name: "Т-46-1",
    nation: "ussr",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 4,
    abilityText:
      "",
  },
  {
    id: "t-12",
    name: "Т-12",
    nation: "ussr",
    class: "medium",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText:
      "",
    combatAbilities: { blitz: true },
  },
  {
    id: "su_5_2",
    name: "СУ-5-2",
    nation: "ussr",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 0,
    hp: 3,
    range: 2,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "amo_f15",
    name: "АМО Ф15",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      hqHealPerTurn: 1,
    },
    abilityText:
      "Даёт +1 топливо и даёт штабу +1 HP в начале хода.",
  },
  {
    id: "ford_aa_ammo",
    name: "Форд-АА",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      drawEveryTurns: 3,
      healRandomUnitPerTurn: 1,
    },
    abilityText:
      "Лечит 1 повреждённый юнит каждый ход и добирает карту каждый третий ход.",
  },
  {
    id: "gaz_55_ambulance",
    name: "ГАЗ-55",
    nation: "ussr",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 7,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "medical",
    supportEffects: {
      healRandomUnitPerTurn: 2,
      hqHealPerTurn: 1,
    },
    abilityText:
      "В начале хода лечит 2 HP повреждённому юниту и даёт штабу +1 HP.",
  },
  {
    id: "panzer_iv",
    name: "Panzer IV G",
    nation: "germany",
    class: "medium",
    rarity: "common",
    cost: 3,
    attack: 4,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "tiger_i",
    name: "Tiger I",
    nation: "germany",
    class: "heavy",
    rarity: "rare",
    cost: 6,
    attack: 5,
    armor: 2,
    hp: 9,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText:
      "",
  },
  // ——— Кампания «Первые Пантеры» ———
  // Трофейный Т-34 учебно-испытательной части Pz. Kummersdorf: наклонная броня
  // отражает снаряды коротких прототипных пушек, но экипаж — учебный (низкая
  // инициатива). Показывает игроку, ПОЧЕМУ ранний прототип недостаточно хорош.
  {
    id: "t34_beute",
    name: "Т-34 beute",
    nation: "germany",
    class: "medium",
    rarity: "common",
    cost: 4,
    attack: 3,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 2,
    captured: true,
    combatAbilities: { frontalArmor: { amount: 1 } },
    abilityText: "",
  },
  // Прототип Daimler-Benz VK 30.01(D) — «клон Т-34» с короткой 75-мм L/24.
  // Наклонный лоб держит удар, но пушка отскакивает от Т-34 (нет «длинного
  // ствола»), а сырой двигатель быстро перегревается.
  {
    id: "vk3001_db",
    name: "VK 30.01 (D)",
    nation: "germany",
    class: "medium",
    rarity: "uncommon",
    level: 3,
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 2,
    prototype: true,
    combatAbilities: {
      frontalArmor: { amount: 1 },
      overheat: { deploymentDamage: { min: 0, max: 3 } },
    },
    abilityText: "",
  },
  // Победивший прототип MAN VK 30.02(M) — база «Пантеры»: наклонная лобовая
  // броня и всё ещё ненадёжный, перегревающийся двигатель.
  {
    id: "vk3002_man",
    name: "VK 30.02 (M)",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    level: 4,
    cost: 4,
    attack: 3,
    armor: 1,
    hp: 5,
    range: 2,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    prototype: true,
    combatAbilities: {
      frontalArmor: { amount: 1 },
      overheat: { deploymentDamage: { min: 0, max: 3 } },
    },
    abilityText: "",
  },
  // Panther Ausf. D — первая серийная. Крепкий лоб, но сырой двигатель: под
  // Курском большинство машин перегревалось ещё на марше (повышенный урон при
  // передвижении).
  {
    id: "panther_d",
    name: "Panther D",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    level: 4,
    cost: 6,
    attack: 4,
    armor: 2,
    hp: 6,
    range: 2,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    combatAbilities: {
      frontalArmor: { amount: 1 },
      overheat: { deploymentDamage: { min: 0, max: 3 }, moveDamage: 2 },
    },
    abilityText: "",
  },
  // Panther Ausf. A — доработанная. Крепче лоб, чуть надёжнее двигатель (порог
  // перегрева выше), но борта по-прежнему уязвимы.
  {
    id: "panther_a",
    name: "Panther A",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    level: 7,
    cost: 5,
    attack: 5,
    armor: 2,
    hp: 7,
    range: 2,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    combatAbilities: {
      longGun: { armorIgnored: 2 },
      frontalArmor: { amount: 3 },
      flankVulnerable: { amount: 2 },
      overheat: { threshold: 3 },
    },
    abilityText: "",
  },
  // Уникум-награда за всю кампанию: Panther «534» полевой доработки — экипаж
  // вылечил болезни машины. Дальний удар, усиленный лоб и «Дозор».
  {
    id: "panther_534",
    name: "Panther 534",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    level: 7,
    cost: 7,
    attack: 5,
    armor: 3,
    hp: 8,
    range: 2,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    combatAbilities: {
      frontalArmor: { amount: 1 },
      drawWhenAttacked: 1,
    },
    abilityText: "",
  },
  // Бергепантера — БРЭМ на шасси «Пантеры». Безоружная (символическая атака),
  // но чинит всех соседей — полевые машины, тыловые юниты и штаб: тушит пожары,
  // освобождает обездвиженные машины, латает броню («Ремонтная летучка»).
  {
    id: "bergepanther",
    name: "Bergepanther",
    nation: "germany",
    class: "medium",
    rarity: "uncommon",
    level: 4,
    cost: 6,
    attack: 0,
    armor: 1,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 2,
    combatAbilities: { repairAura: { healHp: 1 } },
    abilityText: "",
  },
  {
    id: "stug_iii",
    name: "StuG III G",
    nation: "germany",
    class: "td",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 3,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    abilityText:
      "",
  },
  {
    id: "stug_iii_b",
    name: "StuG III B",
    nation: "germany",
    class: "td",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 2,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    abilityText:
      "",
  },
  {
    id: "m4_sherman",
    name: "M4 Sherman",
    nation: "usa",
    class: "medium",
    rarity: "common",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 3,
    initiative: 4,
    abilityText:
      "",
  },
  {
    id: "m5_stuart",
    name: "M5 Stuart",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 3,
    attack: 4,
    armor: 0,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    abilityText:
      "",
    combatAbilities: { blitz: true },
  },
  {
    id: "m1_combat_car",
    name: "M1 Combat Car",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    abilityText:
      "",
    onPlayEffects: { draw: 1 },
  },
  {
    id: "m2_light_tank",
    name: "M2 Light Tank",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 5,
    abilityText:
      "",
  },
  {
    id: "m2_medium_tank",
    name: "M2 Medium Tank",
    nation: "usa",
    class: "medium",
    rarity: "common",
    cost: 4,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "m3_stuart",
    name: "M3 Stuart",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 3,
    attack: 3,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 5,
    abilityText:
      "",
    combatAbilities: { blitz: true },
  },
  {
    id: "m3_lee",
    name: "M3 Lee",
    nation: "usa",
    class: "medium",
    rarity: "uncommon",
    cost: 6,
    attack: 3,
    armor: 1,
    hp: 7,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "m3_halftrack",
    name: "M3 Half-track",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 6,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      drawEveryTurns: 3,
      returnFire: 1,
    },
    abilityText:
      "Даёт +1 топливо, добирает карту каждый третий ход.",
  },
  {
    id: "dodge_wc54",
    name: "Dodge WC-54",
    nation: "usa",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "medical",
    supportEffects: {
      healRandomUnitPerTurn: 1,
      hqHealPerTurn: 1,
    },
    abilityText:
      "Лечит 1 повреждённый юнит и восстанавливает +1 прочности штаба в начале каждого хода.",
  },
  {
    id: "churchill",
    name: "Churchill",
    nation: "uk",
    class: "heavy",
    rarity: "uncommon",
    cost: 4,
    attack: 2,
    armor: 3,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "marder_iii",
    name: "Marder III",
    nation: "germany",
    class: "td",
    rarity: "common",
    cost: 3,
    attack: 4,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    abilityText:
      "",
  },
  {
    id: "wespe",
    name: "Wespe",
    nation: "germany",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 3,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "su_122",
    name: "СУ-122",
    nation: "ussr",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 3,
    movement: 1,
    fuelGeneration: 0,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "tk_3",
    name: "TK-3",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    abilityText: "",
    onPlayEffects: { hqProtection: 1 },
  },
  {
    id: "tks",
    name: "TKS",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    abilityText: "",
    onPlayEffects: { hqProtection: 1 },
  },
  {
    id: "tks_20mm",
    name: "TKS 20 mm",
    nation: "poland",
    class: "td",
    rarity: "uncommon",
    cost: 2,
    attack: 3,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "tkf",
    name: "TKF",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 5,
    abilityText: "",
  },
  {
    id: "tkw",
    name: "TKW",
    nation: "poland",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 5,
    abilityText: "",
  },
  {
    id: "tkd",
    name: "TKD",
    nation: "poland",
    class: "spg",
    rarity: "uncommon",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 2,
    range: 3,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "tks_d",
    name: "TKS-D",
    nation: "poland",
    class: "td",
    rarity: "uncommon",
    cost: 2,
    attack: 4,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "tp7_dwuwiezowy",
    name: "7TP dwuwieżowy",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 0,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "tp7_jednowiezowy",
    name: "7TP jednowieżowy",
    nation: "poland",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 3,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "tp7_wzmocniony",
    name: "7TP wzmocniony",
    nation: "poland",
    class: "light",
    rarity: "rare",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 6,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "tp9",
    name: "9TP",
    nation: "poland",
    class: "light",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "tp10",
    name: "10TP",
    nation: "poland",
    class: "medium",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 4,
    combatAbilities: { blitz: true },
    abilityText: "",
  },
  {
    id: "tp14",
    name: "14TP",
    nation: "poland",
    class: "medium",
    rarity: "rare",
    cost: 4,
    attack: 4,
    armor: 2,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "vickers_e_type_a",
    name: "Vickers E A",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "vickers_e_type_b",
    name: "Vickers E B",
    nation: "poland",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 3,
    armor: 0,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "renault_ft",
    name: "Renault FT",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 1,
    hp: 2,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
    onPlayEffects: { draw: 1 },
  },
  {
    id: "renault_r35",
    name: "Renault R35",
    nation: "poland",
    class: "light",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 2,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "hotchkiss_h35",
    name: "Hotchkiss H35",
    nation: "poland",
    class: "light",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 2,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "pociag_pancerny_danuta",
    name: "Pociąg pancerny “Danuta”",
    nation: "poland",
    class: "heavy",
    rarity: "rare",
    cost: 5,
    attack: 5,
    armor: 3,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 3,
    initiative: 1,
    abilityText: "Бронепоезд.",
  },
  {
    id: "pociag_pancerny_smialy",
    name: "Pociąg pancerny “Śmiały”",
    nation: "poland",
    class: "heavy",
    rarity: "rare",
    cost: 5,
    attack: 6,
    armor: 2,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 1,
    abilityText: "Бронепоезд.",
  },

  // === 1. Panzer-Division — ранние немецкие машины (Польша 1939) ===

  // Лёгкие танки
  {
    id: "leichttraktor",
    name: "Leichttraktor",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText: "При выходе на поле добирает карту.",
    onPlayEffects: { draw: 1 },
  },
  {
    id: "pzkpfw_i_ausf_a",
    name: "Panzer I A",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 2,
    armor: 0,
    hp: 1,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText: "",
    onPlayEffects: { draw: 1 },
  },
  {
    id: "pzkpfw_i_ausf_b",
    name: "Panzer I B",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText: "",
    onPlayEffects: { hqProtection: 1 },
  },
  {
    id: "pzkpfw_ii_ausf_c",
    name: "Panzer II C",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText: "",
    combatAbilities: { blitz: true },
  },
  {
    id: "pzkpfw_ii_ausf_f",
    name: "Panzer II F",
    nation: "germany",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 3,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "pzkpfw_ii_ausf_d",
    name: "Panzer II D",
    nation: "germany",
    class: "light",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    combatAbilities: { blitz: true },
    abilityText:
      "",
    
  },
  {
    id: "panzer_35t",
    name: "Panzer 35(t)",
    nation: "germany",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 1,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "panzer_38t",
    name: "Panzer 38(t)",
    nation: "germany",
    class: "light",
    rarity: "uncommon",
    cost: 5,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
    combatAbilities: { blitz: true },
  },
  {
    id: "pzbef_i",
    name: "Funk Panzer I",
    nation: "germany",
    class: "light",
    rarity: "uncommon",
    level: 3,
    cost: 4,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 2,
    initiative: 4,
    abilityText: "",
    onPlayEffects: { draw: 1, hqProtection: 1 },
  },

  // Средние танки
  {
    id: "pzkpfw_iii_ausf_a",
    name: "Panzer III A",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "pzkpfw_iii_ausf_d",
    name: "Panzer III D",
    nation: "germany",
    class: "medium",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 2,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "pzkpfw_iii_ausf_e",
    name: "Panzer III E",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    cost: 4,
    attack: 2,
    armor: 2,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "pzkpfw_iv_ausf_a",
    name: "Panzer IV A",
    nation: "germany",
    class: "medium",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "pzkpfw_iv_ausf_b",
    name: "Panzer IV B",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    cost: 5,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },

  // Противотанковые САУ
  {
    id: "panzerjaeger_i",
    name: "PzJäger I",
    nation: "germany",
    class: "td",
    rarity: "uncommon",
    cost: 2,
    attack: 4,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText: "",
  },
  
  {
    id: "panzerjaeger_38t_early",
    name: "Marder II",
    nation: "germany",
    class: "td",
    rarity: "rare",
    cost: 3,
    attack: 5,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText: "",
  },

  // Самоходные орудия
  {
    id: "sig_33_pzi",
    name: "Bison I",
    nation: "germany",
    class: "spg",
    rarity: "uncommon",
    cost: 4,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 3,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "sig_33_pzii",
    name: "Bison II",
    nation: "germany",
    class: "spg",
    rarity: "rare",
    cost: 4,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 3,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    abilityText: "",
  },

  // Тяжёлые / прототипы
  {
    id: "neubaufahrzeug",
    name: "Nb.Fz.",
    nation: "germany",
    class: "heavy",
    rarity: "rare",
    cost: 7,
    attack: 5,
    armor: 2,
    hp: 7,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "grosstraktor",
    name: "Großtraktor",
    nation: "germany",
    class: "heavy",
    rarity: "rare",
    cost: 5,
    attack: 2,
    armor: 2,
    hp: 9,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText: "",
  },
  // === Поддержка — Польша 1939 (конкретная техника, а не колонны) ===
  {
    id: "armata_75mm",
    name: "Armata 75 mm",
    nation: "poland",
    class: "spg",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 7,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 1,
      hqDamageRedirect: 1,
    },
    abilityText: "Увеличивает атаку штаба и принимает часть урона на себя.",
  },
  {
    id: "haubica_100mm",
    name: "Haubica 100 mm",
    nation: "poland",
    class: "spg",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 5,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 1,
      hqDamageRedirect: 2,
    },
    abilityText: "Сильное прикрытие штаба при обороне.",
  },
  {
    id: "ciagacz_c4p",
    name: "Ciągacz C4P",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 6,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 2,
    },
    abilityText: "Даёт +2 топлива в ход.",
  },
  {
    id: "fiat_508_junak",
    name: "Fiat 508 Junak",
    nation: "poland",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      drawEveryTurns: 2,
    },
    abilityText: "Позволяет добирать дополнительную карту каждый второй ход.",
  },
  {
    id: "karetka_sanitarnaya",
    name: "Karetka sanitar",
    nation: "poland",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 3,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "medical",
    supportEffects: {
      healRandomUnitPerTurn: 1,
    },
    abilityText: "В начале хода восстанавливает 1 HP случайному повреждённому юниту.",
  },

  // === Поддержка — Германия, ранние (Польша 1939, конкретная техника) ===
  {
    id: "lefh_18",
    name: "leFH 18",
    nation: "germany",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 0,
    armor: 0,
    hp: 6,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 2,
    },
    abilityText: "Увеличивает атаку штаба на 2",
  },
  {
    id: "leig_18",
    name: "leIG 18",
    nation: "germany",
    class: "spg",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 1,
    },
    abilityText: "+1 к атаке штаба.",
  },
  {
    id: "mercedes_g3a",
    name: "Mercedes G3a",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      hqAttackBonus: 1,
    },
    abilityText: "Даёт +1 топливо и помогает штабу вести огонь точнее: +1 к атаке.",
  },
  {
    id: "adler_type_10_n",
    name: "Adler 10N",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 1,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      hqAttackBonus: 1,
    },
    abilityText: "Даёт +1 к атаке штаба и +1 топливо каждый ход.",
  },
  {
    id: "sanitaetskraftwagen",
    name: "Sanitätskraftwgn",
    nation: "germany",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "medical",
    supportEffects: {
      healRandomUnitPerTurn: 1,
      hqDamageRedirect: 1,
    },
    abilityText: "В начале хода лечит 1 повреждённый юнит.",
  },

  // === СССР — ранний период и эксперименты (1931–1941) ===
  {
    id: "t27",
    name: "Т-27",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 0,
    hp: 1,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 6,
    abilityText: "",
    onPlayEffects: { suppressEnemyIndirect: true },
    combatAbilities: { blitz: true },
  },
  {
    id: "su18",
    name: "СУ-18",
    nation: "ussr",
    class: "spg",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 2,
    movement: 1,
    fuelGeneration: 0,
    initiative: 1,
    abilityText: "",
    onPlayEffects: {
      deployDamage: { amount: 1, scope: "rear" },
    },
  },
  {
    id: "bt_7m",
    name: "БТ-7М",
    nation: "ussr",
    class: "light",
    rarity: "rare",
    cost: 4,
    attack: 4,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 4,
    abilityText:
      "",
    combatAbilities: { blitz: true, drawWhenAttacked: 1 },
  },
  {
    id: "bt_sv",
    name: "БТ-СВ «Черепаха»",
    nation: "ussr",
    class: "light",
    rarity: "rare",
    cost: 3,
    attack: 2,
    armor: 2,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText: "",
    combatAbilities: { blitz: true },
  },
  {
    id: "a20",
    name: "А-20",
    nation: "ussr",
    class: "light",
    rarity: "rare",
    cost: 4,
    attack: 3,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 4,
    abilityText: "",
    combatAbilities: { blitz: true },
  },
  {
    id: "t111",
    name: "Т-111",
    nation: "ussr",
    class: "light",
    rarity: "rare",
    cost: 3,
    attack: 2,
    armor: 2,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "t29",
    name: "Т-29",
    nation: "ussr",
    class: "medium",
    rarity: "rare",
    cost: 4,
    attack: 3,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
    combatAbilities: { blitz: true },
  },
  {
    id: "smk",
    name: "СМК",
    nation: "ussr",
    class: "heavy",
    rarity: "rare",
    cost: 6,
    attack: 4,
    armor: 2,
    hp: 9,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "t100",
    name: "Т-100",
    nation: "ussr",
    class: "heavy",
    rarity: "rare",
    cost: 6,
    attack: 4,
    armor: 2,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "kv2",
    name: "КВ-2",
    nation: "ussr",
    class: "heavy",
    rarity: "rare",
    level: 6,
    cost: 7,
    attack: 6,
    armor: 2,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "at1",
    name: "АТ-1",
    nation: "ussr",
    class: "td",
    rarity: "uncommon",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "su14",
    name: "СУ-14",
    nation: "ussr",
    class: "spg",
    rarity: "rare",
    cost: 6,
    attack: 4,
    armor: 0,
    hp: 4,
    range: 3,
    movement: 1,
    fuelGeneration: 0,
    initiative: 1,
    abilityText: "",
  },
  {
    id: "gun_53k",
    name: "45-мм 53-К",
    nation: "ussr",
    class: "spg",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      supportLineCover: 2,
    },
    abilityText:
      "Встречает рейды на тыл огнём и принимает дистанционные удары на себя.",
  },
  {
    id: "t18_dot",
    name: "Т-18 ДОТ",
    nation: "ussr",
    class: "spg",
    rarity: "common",
    level: 1,
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 5,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      supportLineCover: 1,
    },
    abilityText: "Противотанковый заслон.",
  },
  {
    id: "gun_76_1927",
    name: "76-мм обр. 1927",
    nation: "ussr",
    class: "spg",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 5,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 1,
    },
    abilityText: "Полковая пушка: +1 к атаке штаба.",
  },
  {
    id: "gun_m30",
    name: "122-мм М-30",
    nation: "ussr",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 0,
    armor: 0,
    hp: 7,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 2,
    },
    abilityText: "Дивизионная гаубица: +2 к атаке штаба.",
  },
  {
    id: "repair_letuchka",
    name: "Летучка тип «А»",
    nation: "ussr",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 7,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "medical",
    supportEffects: {
      healRandomUnitPerTurn: 1,
    },
    abilityText: "Ремонтная мастерская",
  },

  // === США — ранний период и эксперименты (1931–1942) ===
  {
    id: "m2a4",
    name: "M2A4",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 4,
    abilityText: "",
  },
  {
    id: "christie_t3",
    name: "Christie T3",
    nation: "usa",
    class: "light",
    rarity: "rare",
    cost: 3,
    attack: 2,
    armor: 0,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    abilityText: "",
    combatAbilities: { blitz: true },
  },
  {
    id: "marmon_ctls",
    name: "Marmon CTLS",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 4,
    abilityText: "",
    onPlayEffects: { draw: 1 },
  },
  {
    id: "lvt1",
    name: "LVT-1 Alligator",
    nation: "usa",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 1,
    armor: 0,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 0,
    initiative: 3,
    abilityText: "",
    onPlayEffects: { hqProtection: 1 },
  },
  {
    id: "m2a1_medium",
    name: "M2A1 Medium",
    nation: "usa",
    class: "medium",
    rarity: "common",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "m6_heavy",
    name: "M6 Heavy",
    nation: "usa",
    class: "heavy",
    rarity: "rare",
    cost: 7,
    attack: 5,
    armor: 2,
    hp: 9,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "t14_assault",
    name: "T14 Tank",
    nation: "usa",
    class: "heavy",
    rarity: "rare",
    cost: 5,
    attack: 3,
    armor: 2,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "m3_gmc",
    name: "M3 GMC",
    nation: "usa",
    class: "td",
    rarity: "uncommon",
    cost: 3,
    attack: 4,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText: "Опасен вблизи.",
  },
  {
    id: "m6_gmc_fargo",
    name: "M6 Fargo",
    nation: "usa",
    class: "td",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "t18_hmc",
    name: "T18 HMC",
    nation: "usa",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 5,
    range: 3,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "t19_hmc",
    name: "T19 HMC",
    nation: "usa",
    class: "spg",
    rarity: "uncommon",
    cost: 4,
    attack: 3,
    armor: 0,
    hp: 3,
    range: 3,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "gun_37mm_m3",
    name: "37-мм M3",
    nation: "usa",
    class: "spg",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 3,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      supportLineCover: 1,
    },
    onPlayEffects: {
      deployDamage: { amount: 1, scope: "random" },
    },
    abilityText:
      "",
  },
  {
    id: "gun_75_pack",
    name: "75-мм Pack M1A1",
    nation: "usa",
    class: "spg",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 3,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 1,
      returnFire: 1,
    },
    abilityText: "Вьючная гаубица: +1 к атаке штаба и самооборона.",
  },
  {
    id: "gun_105_m2a1",
    name: "105-мм M2A1",
    nation: "usa",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 2,
      supportLineCover: 1,
    },
    abilityText: "",
  },
  {
    id: "willys_mb",
    name: "Bantam BRC 40",
    nation: "usa",
    class: "light",
    rarity: "uncommon",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 3,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      drawEveryTurns: 2,
    },
    abilityText: "Связная разведмашина: +1 топлива и добор карты каждый второй ход.",
  },

  // === Пополнение: штабные машины и переходные модели (1940–1942) ===
  {
    id: "gaz_m1",
    name: "ГАЗ-М1",
    nation: "ussr",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 5,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fetchSupportCardEveryTurns: 2,
    },
    abilityText:
      "Каждый второй ход доставляет в руку случайную карту поддержки из колоды.",
  },
  {
    id: "t34_1940",
    name: "Т-34 1940",
    nation: "ussr",
    class: "medium",
    rarity: "uncommon",
    cost: 4,
    attack: 3,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "kv1_1940",
    name: "КВ-1 обр. 1940",
    nation: "ussr",
    class: "heavy",
    rarity: "rare",
    cost: 5,
    attack: 3,
    armor: 2,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText:
      "",
  },
  {
    id: "horch_830r",
    name: "Horch 830R",
    nation: "germany",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      hqAttackBonus: 1,
      hqDamageRedirect: 1,
    },
    abilityText:
      "Штабной кабриолет: +1 к атаке штаба и перехватывает 1 урон по штабу.",
  },
  {
    id: "porsche_823",
    name: "Porsche-823",
    nation: "germany",
    class: "light",
    rarity: "rare",
    cost: 3,
    attack: 0,
    armor: 1,
    hp: 5,
    range: 0,
    movement: 0,
    fuelGeneration: 1,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      tankScreenClasses: ["light", "medium"],
    },
    abilityText:
      "+1 топлива. Перехватывает удары по лёгким и средним танкам, принимая огонь на себя.",
  },
  {
    id: "krupp_l3h163",
    name: "Krupp L3H163",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 6,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
    },
    abilityText: "Армейский грузовик: +1 топлива каждый ход.",
  },
  {
    id: "ford_gpa",
    name: "Ford GPA",
    nation: "usa",
    class: "light",
    rarity: "uncommon",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 6,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      drawEveryTurns: 3,
    },
    abilityText:
      "Амфибия-снабженец: +1 топлива в ход и добирает карту каждый третий ход.",
  },
  {
    id: "m5_hst",
    name: "M5 HST",
    nation: "usa",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 2,
    },
    onPlayEffects: {
      fetchToHand: {
        label: "артиллерия",
        match: { classes: ["td", "spg"], supportRoles: ["artillery"] },
      },
    },
    abilityText:
      "+2 топлива. При выходе добавляет в руку артиллерию тыла из колоды.",
  },
  {
    id: "sherman_early",
    name: "M4A1 Sherman",
    nation: "usa",
    class: "medium",
    rarity: "uncommon",
    cost: 4,
    attack: 3,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },

  // === Кампания «Лавриненко» — СССР, осень 1941 ===

  // Сигнатурная карта-награда: личный Т-34 аса. Уникум (выдаётся за прохождение
  // кампании). Блиц + разведка при выходе; в связке со штабом «Танковая засада»
  // превращается в неуязвимую огневую точку.
  {
    id: "t34_lavrinenko",
    name: "Т-34 Лавриненко",
    nation: "ussr",
    class: "medium",
    rarity: "rare",
    level: 7,
    cost: 4,
    attack: 4,
    armor: 2,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 4,
    combatAbilities: { blitz: true, frontalArmor: { amount: 1 } },
    onPlayEffects: { draw: 1 },
    abilityText:
      "Машина аса Лавриненко",
  },
  {
    id: "t34_1941",
    name: "Т-34/76 1941",
    nation: "ussr",
    class: "medium",
    rarity: "uncommon",
    level: 3,
    cost: 4,
    attack: 2,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "t34_stz",
    name: "Т-34 (СТЗ)",
    nation: "ussr",
    class: "medium",
    rarity: "common",
    level: 3,
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText:
      "При выходе на поле боя добавляет в руку случайный Т-34 из колоды.",
  },
  {
    id: "bt_7_command",
    name: "КБТ-7",
    nation: "ussr",
    class: "light",
    rarity: "uncommon",
    level: 6,
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    combatAbilities: { tankDefenseAura: 1 },
    onPlayEffects: { draw: 1 },
    abilityText:
      "Пока в строю — все ваши танки получают −1 к каждому входящему удару.",
  },
  {
    id: "t60",
    name: "Т-60",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 2,
    fuelGeneration: 1,
    initiative: 5,
    combatAbilities: { blitz: true },
    abilityText: "",
  },
  {
    id: "zis_30",
    name: "ЗИС-30",
    nation: "ussr",
    class: "td",
    rarity: "uncommon",
    level: 3,
    cost: 3,
    attack: 5,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 3,
    abilityText:
      "",
  },
  {
    id: "zis_5_ammo",
    name: "ЗИС-5",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 5,
    range: 0,
    movement: 0,
    fuelGeneration: 2,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      drawEveryTurns: 3,
    },
    abilityText: "+1 топлива и добор карты каждый третий ход",
  },
  {
    id: "parm_workshop",
    name: "ПАРМ (летучка)",
    nation: "ussr",
    class: "light",
    rarity: "uncommon",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 6,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "medical",
    supportEffects: {
      healRandomUnitPerTurn: 2,
    },
    abilityText: "В начале хода чинит 2 HP случайному повреждённому юниту.",
  },
  {
    id: "m72_recon",
    name: "Мотоцикл М-72",
    nation: "ussr",
    class: "light",
    rarity: "common",
    cost: 4,
    attack: 0,
    armor: 0,
    hp: 2,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      drawEveryTurns: 2,
      returnFire: 1,
    },
    onPlayEffects: {
      suppressEnemyIndirect: true,
    },
    abilityText:
      "Добор карты каждый второй ход и самооборона 1. Пока мотоцикл в строю, вражеские штаб и САУ не могут атаковать.",
  },

  // === Кампания «Лавриненко» — Германия, 1941 ===
  {
    id: "pzkpfw_iii_ausf_h",
    name: "Panzer III H",
    nation: "germany",
    class: "medium",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 2,
    hp: 4,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "pzkpfw_iii_ausf_j",
    name: "Panzer III J",
    nation: "germany",
    class: "medium",
    rarity: "rare",
    cost: 4,
    attack: 2,
    armor: 2,
    hp: 6,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 3,
    abilityText: "",
  },
  {
    id: "pzkpfw_iv_ausf_e",
    name: "Panzer IV E",
    nation: "germany",
    class: "medium",
    rarity: "uncommon",
    cost: 4,
    attack: 2,
    armor: 1,
    hp: 5,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    abilityText: "",
  },
  {
    id: "sdkfz_251",
    name: "SdKfz 251",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 3,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 1,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      hqAttackBonus: 1,
      returnFire: 1,
    },
    abilityText:
      "+1 топлива, +1 к атаке штаба и отвечает огнём на ближние атаки.",
  },
  {
    id: "krad_bmw",
    name: "BMW R75",
    nation: "germany",
    class: "light",
    rarity: "common",
    cost: 1,
    attack: 0,
    armor: 0,
    hp: 3,
    range: 0,
    movement: 0,
    fuelGeneration: 1,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "transport",
    supportEffects: {
      fuelPerTurn: 1,
      drawEveryTurns: 2,
      returnFire: 1,
    },
    abilityText:
      "Добор карты каждый второй ход. отвечает на ближние атаки. +1 топливо",
  },
  {
    id: "pak36",
    name: "3,7 cm Pak 36",
    nation: "germany",
    class: "spg",
    rarity: "common",
    cost: 2,
    attack: 0,
    armor: 0,
    hp: 3,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      supportLineCover: 3,
    },
    abilityText:
      "Встречает рейды на тыл огнём и принимает дистанционные удары на себя.",
  },
  {
    id: "pak38",
    name: "5 cm Pak 38",
    nation: "germany",
    class: "spg",
    rarity: "uncommon",
    cost: 3,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 1,
      supportLineCover: 2,
    },
    abilityText:
      "+1 к атаке штаба и противотанковый заслон тыловой линии.",
  },

  // === Миссия-трейлер «Зверобой. Поныри» (Курская дуга 1943) ===
  {
    id: "su_152",
    name: "СУ-152",
    nation: "ussr",
    class: "td",
    rarity: "rare",
    level: 7,
    cost: 6,
    attack: 5,
    armor: 2,
    hp: 6,
    range: 2,
    movement: 1,
    fuelGeneration: 1,
    initiative: 2,
    abilityText:
      "",
  },
  {
    id: "ferdinand",
    name: "Ferdinand",
    nation: "germany",
    class: "td",
    rarity: "rare",
    level: 7,
    cost: 7,
    attack: 6,
    armor: 3,
    hp: 10,
    range: 1,
    movement: 1,
    fuelGeneration: 1,
    initiative: 1,
    abilityText:
      "Лобовая броня: −2 урона при ударе со стороны вражеского штаба. Фланг и тыл пробиваются.",
  },
  {
    id: "pzkpfw_iii_ausf_f",
    name: "Panzer III F",
    nation: "germany",
    class: "medium",
    rarity: "uncommon",
    cost: 4,
    attack: 3,
    armor: 2,
    hp: 3,
    range: 1,
    movement: 1,
    fuelGeneration: 0,
    initiative: 2,
    abilityText: "",
  },

  // === Бронеавтомобили (ранние образцы, до 1942) — высокомобильные
  // слабобронированные рейдеры ===
  // Класс «armored_car»: за ход проходит до 3 клеток по прямой или 2 раза по
  // диагонали; может атаковать дважды за ход, если бьёт по тылу или штабу; по
  // обычным юнитам урон −1 с фронта и +1 с фланга/тыла (по САУ и другим
  // бронеавтомобилям — стандартный). Слабая броня — берут манёвром, не лобовой.
  // Пушечные машины несут «Контрбатарею»: глушат вражеские САУ и штаб.

  // --- СССР ---
  {
    id: "d8",
    name: "Д-8", // 1932, малый пулемётный
    nation: "ussr",
    class: "armored_car",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    onPlayEffects: { draw: 1 },
    abilityText: "",
  },
  {
    id: "fai",
    name: "ФАИ", // 1933
    nation: "ussr",
    class: "armored_car",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    onPlayEffects: { draw: 1 },
    abilityText: "",
  },
  {
    id: "ba_20_ac",
    name: "БА-20", // 1936
    nation: "ussr",
    class: "armored_car",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    combatAbilities: { raidDraw: 1 },
    abilityText: "",
  },
  {
    id: "ba_6_ac",
    name: "БА-6", // 1936
    nation: "ussr",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    combatAbilities: { drawWhenAttacked: 1 },
    abilityText: "",
  },
  {
    id: "ba_10_ac",
    name: "БА-10", // 1938, 45-мм пушка
    nation: "ussr",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 2,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    onPlayEffects: { suppressEnemyIndirect: true },
    abilityText:
      "",
  },
  {
    id: "ba_11_ac",
    name: "БА-11", // 1940
    nation: "ussr",
    class: "armored_car",
    rarity: "rare",
    cost: 4,
    attack: 3,
    armor: 2,
    hp: 5,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 4,
    combatAbilities: { armorVsClass: { class: "light", amount: 1 } },
    abilityText: "",
  },

  // --- Германия ---
  {
    id: "kfz_13",
    name: "Kfz. 13 Adler", // 1932
    nation: "germany",
    class: "armored_car",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    onPlayEffects: { draw: 1 },
    abilityText: "",
  },
  {
    id: "adgz",
    name: "ADGZ", // Steyr ADGZ, 1934 (тяжёлый колёсный)
    nation: "germany",
    class: "armored_car",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 5,
    abilityText: "",
  },
  {
    id: "sdkfz_221",
    name: "Sd.Kfz. 221", // 1935
    nation: "germany",
    class: "armored_car",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    combatAbilities: { raidDraw: 1 },
    abilityText: "",
  },
  {
    id: "sdkfz_222",
    name: "Sd.Kfz. 222", // 1936, 2-см автопушка
    nation: "germany",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 2,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    combatAbilities: { drawWhenAttacked: 1 },
    abilityText: "",
  },
  {
    id: "sdkfz_231",
    name: "Sd.Kfz. 231 (6-Rad)", // 1936, тяжёлый разведбронеавтомобиль
    nation: "germany",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 1,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    onPlayEffects: { suppressEnemyIndirect: true },
    abilityText:
      "",
  },
  {
    id: "sdkfz_263",
    name: "Sd.Kfz. 263 Funk", // 1938, командно-радийная
    nation: "germany",
    class: "armored_car",
    rarity: "rare",
    cost: 4,
    attack: 0,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 5,
    combatAbilities: { tankDefenseAura: 1 },
    abilityText:
      "Пока в строю, каждый ваш танк получает −1 к входящему урону.",
  },

  // --- США ---
  {
    id: "m1_armored_car",
    name: "M1 Armored Car", // 1931 (Cunningham)
    nation: "usa",
    class: "armored_car",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    onPlayEffects: { draw: 1 },
    abilityText: "",
  },
  {
    id: "m2_scout_car",
    name: "M2 Scout Car", // 1935
    nation: "usa",
    class: "armored_car",
    rarity: "common",
    cost: 2,
    attack: 1,
    armor: 1,
    hp: 3,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    combatAbilities: { raidDraw: 1 },
    abilityText: "",
  },
  {
    id: "m3_scout_car",
    name: "M3 Scout Car", // 1939 (White)
    nation: "usa",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    combatAbilities: { drawWhenAttacked: 1 },
    abilityText: "",
  },
  {
    id: "m3a1_scout_car",
    name: "M3A1 Scout Car", // 1940
    nation: "usa",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 5,
    onPlayEffects: { hqProtection: 1 },
    abilityText: "",
  },
  {
    id: "t13_armored_car",
    name: "T13 Armored Car", // 1941 (прототип)
    nation: "usa",
    class: "armored_car",
    rarity: "rare",
    cost: 4,
    attack: 3,
    armor: 2,
    hp: 5,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 4,
    combatAbilities: { armorVsClass: { class: "light", amount: 1 } },
    abilityText: "",
  },

  // --- Польша ---
  {
    id: "wz28",
    name: "wz. 28", // 1928, полугусеничный
    nation: "poland",
    class: "armored_car",
    rarity: "common",
    cost: 1,
    attack: 1,
    armor: 0,
    hp: 2,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 6,
    onPlayEffects: { draw: 1 },
    abilityText: "",
  },
  {
    id: "wz34",
    name: "wz. 34", // 1934
    nation: "poland",
    class: "armored_car",
    rarity: "common",
    cost: 2,
    attack: 2,
    armor: 0,
    hp: 3,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 6,
    combatAbilities: { raidDraw: 1 },
    abilityText: "",
  },
  {
    id: "wz34_ii",
    name: "wz. 34-II", // 1934, 37-мм пушка
    nation: "poland",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    combatAbilities: { drawWhenAttacked: 1 },
    abilityText: "",
  },
  {
    id: "wz29_ursus",
    name: "wz. 29 Ursus", // 1929, 37-мм пушка
    nation: "poland",
    class: "armored_car",
    rarity: "uncommon",
    cost: 3,
    attack: 3,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 0,
    initiative: 5,
    onPlayEffects: { suppressEnemyIndirect: true },
    abilityText:
      "",
  },
  {
    id: "wz29_dow",
    name: "wz. 29 dowódczy", // ~1931, командная
    nation: "poland",
    class: "armored_car",
    rarity: "rare",
    cost: 4,
    attack: 2,
    armor: 1,
    hp: 4,
    range: 1,
    movement: 3,
    fuelGeneration: 1,
    initiative: 5,
    combatAbilities: { tankDefenseAura: 1 },
    abilityText:
      "Командная машина: пока в строю, каждый ваш танк получает −1 к входящему урону.",
  },

  // === Кампания «Одинокий КВ» (Расейняй, июнь 1941) ===
  {
    id: "flak_88",
    name: "8,8 cm Flak 36",
    nation: "germany",
    class: "spg",
    rarity: "rare",
    level: 6,
    cost: 5,
    attack: 0,
    armor: 0,
    hp: 4,
    range: 0,
    movement: 0,
    fuelGeneration: 0,
    initiative: 1,
    deploymentZone: "support",
    supportRole: "artillery",
    supportEffects: {
      hqAttackBonus: 3,
      supportLineCover: 2,
    },
    abilityText:
      "«Ахт-ахт»: +3 к атаке штаба — единственное орудие, пробивающее любую броню.",
  },
  {
    id: "kv1_raseiniai",
    name: "КВ-1Э «Расейняйский»",
    nation: "ussr",
    class: "heavy",
    rarity: "rare",
    level: 5,
    cost: 6,
    attack: 4,
    armor: 3,
    hp: 8,
    range: 1,
    movement: 1,
    fuelGeneration: 2,
    initiative: 2,
    combatAbilities: {
      frontalArmor: { amount: 1 },
      armorVsClass: { class: "light", amount: 1 },
    },
    abilityText:
      "Экранированный КВ, двое суток державший дорогу на Расейняй.",
  },
];

// Топливо на поле боя генерируют только тыловые (support) юниты и штаб
// (см. calculateFuelGeneration). У полевых юнитов прирост топлива обнуляется,
// чтобы значения не вводили в заблуждение бота/веса колод и на карте не
// рисовалась лишняя канистра «+N FUEL».
export const cards: TankCard[] = rawCards.map((card) =>
  card.deploymentZone === "support" ? card : { ...card, fuelGeneration: 0 }
);

/**
 * Special abilities layered onto existing cards. Kept as one readable table so
 * combat flavor/balance can be tuned in a single place without touching the
 * large card literals above. Overrides are merged into a card's existing
 * `combatAbilities` / `onPlayEffects`, so cards that already carry an ability
 * keep it. A handful of plain "base" cards per class are intentionally absent
 * here and stay ability-free.
 */
type AbilityOverride = Pick<
  TankCard,
  "combatAbilities" | "onPlayEffects" | "costModifiers"
>;

const UNIT_ABILITY_OVERRIDES: Record<string, AbilityOverride> = {
  // ===== Лёгкие танки ===== (база без способностей: МС-1, Т-26 1931, M2 Light)
  t26_1933: { combatAbilities: { drawWhenAttacked: 1 } },
  t26_1938: { combatAbilities: { armorVsClass: { class: "light", amount: 1 } } },
  t46_1: { onPlayEffects: { draw: 1 } },
  m2a4: {
    combatAbilities: { spawnDamageReduction: 1 },
    onPlayEffects: { draw: 1 },
  },
  marmon_ctls: { combatAbilities: { camouflage: true } },
  lvt1: { combatAbilities: { spawnDamageReduction: 1 } },
  m5_stuart: { combatAbilities: { raidDraw: 1 } },
  t111: { combatAbilities: { spawnDamageReduction: 1 } },
  panzer_35t: { combatAbilities: { raidDraw: 1 } },
  pzkpfw_ii_ausf_f: { combatAbilities: { camouflage: true } },
  tkf: { combatAbilities: { camouflage: true } },
  tkw: { combatAbilities: { drawWhenAttacked: 1 } },
  tp7_dwuwiezowy: { onPlayEffects: { hqProtection: 1 } },
  tp7_jednowiezowy: { combatAbilities: { blitz: true } },
  tp7_wzmocniony: { combatAbilities: { armorVsClass: { class: "td", amount: 1 } } },
  tp9: { combatAbilities: { armorVsClass: { class: "medium", amount: 1 } } },
  vickers_e_type_a: { combatAbilities: { attackEqualsHq: true } },
  vickers_e_type_b: { combatAbilities: { raidDraw: 1 } },
  renault_r35: { combatAbilities: { spawnDamageReduction: 1 } },
  hotchkiss_h35: { combatAbilities: { armorVsClass: { class: "light", amount: 1 } } },

  // ===== Средние танки ===== (база: T-34/76, M4 Sherman, Panzer III A)
  t24: { onPlayEffects: { draw: 1 } },
  t28: { combatAbilities: { armorVsClass: { class: "light", amount: 1 } } },
  panzer_iv: { costModifiers: { ifClassPresent: "light", discount: 1 } },
  m2_medium_tank: {
    combatAbilities: { armorVsClass: { class: "light", amount: 1 } },
    onPlayEffects: { hqProtection: 1 },
  },
  m3_lee: {
    combatAbilities: { armorVsClass: { class: "spg", amount: 1 } },
    onPlayEffects: { suppressEnemyIndirect: true },
  },
  tp14: { combatAbilities: { armorVsClass: { class: "medium", amount: 1 } } },
  pzkpfw_iii_ausf_d: { combatAbilities: { drawWhenAttacked: 1 } },
  pzkpfw_iii_ausf_e: { combatAbilities: { spawnDamageReduction: 1 } },
  pzkpfw_iii_ausf_h: { combatAbilities: { armorVsClass: { class: "td", amount: 1 } } },
  pzkpfw_iii_ausf_j: { combatAbilities: { spawnDamageReduction: 1 } },
  pzkpfw_iv_ausf_a: { onPlayEffects: { draw: 1 } },
  pzkpfw_iv_ausf_b: { combatAbilities: { blitz: true } },
  pzkpfw_iv_ausf_e: { costModifiers: { ifClassPresent: "light", discount: 1 } },
  m2a1_medium: { combatAbilities: { attackEqualsHq: true } },
  t34_1940: { costModifiers: { ifClassPresent: "medium", discount: 1 } },
  t34_1941: { combatAbilities: { armorVsClass: { class: "medium", amount: 1 } } },
  t34_stz: {
    onPlayEffects: {
      fetchToHand: { match: { namePrefixes: ["Т-34"] } },
    },
  },
  sherman_early: {
    combatAbilities: { drawWhenAttacked: 1 },
    costModifiers: { ifClassPresent: "light", discount: 1 },
  },

  // ===== Тяжёлые танки ===== (база: KV-1, Churchill)
  t35: { combatAbilities: { armorVsClass: { class: "light", amount: 2 } } },
  tiger_i: {
    combatAbilities: {
      armorVsClass: { class: "medium", amount: 2 },
      frontalArmor: { amount: 1 },
    },
  },
  pociag_pancerny_danuta: { onPlayEffects: { suppressEnemyIndirect: true } },
  pociag_pancerny_smialy: { combatAbilities: { spawnDamageReduction: 2 } },
  neubaufahrzeug: { combatAbilities: { drawWhenAttacked: 1 } },
  grosstraktor: { combatAbilities: { attackEqualsHq: true } },
  smk: { combatAbilities: { armorVsClass: { class: "td", amount: 2 } } },
  t100: { combatAbilities: { spawnDamageReduction: 2 } },
  kv2: { onPlayEffects: { deployDamage: { amount: 2, scope: "random" } } },
  m6_heavy: {
    combatAbilities: {
      armorVsClass: { class: "medium", amount: 2 },
      frontalArmor: { amount: 1 },
    },
  },
  t14_assault: {
    combatAbilities: {
      frontalArmor: { amount: 2 },
      spawnDamageReduction: 1,
    },
  },
  kv1_1940: { combatAbilities: { frontalArmor: { amount: 2 } } },

  // ===== ПТ-САУ ===== (база: StuG III B, АТ-1)
  stug_iii: { combatAbilities: { frontalArmor: { amount: 2 } } },
  marder_iii: { combatAbilities: { camouflage: true } },
  tks_20mm: { combatAbilities: { drawWhenAttacked: 1 } },
  tks_d: { combatAbilities: { camouflage: true } },
  panzerjaeger_38t_early: { combatAbilities: { camouflage: true } },
  m3_gmc: {
    combatAbilities: { drawWhenAttacked: 1 },
    onPlayEffects: { deployDamage: { amount: 1, scope: "random" } },
  },
  m6_gmc_fargo: {
    combatAbilities: {
      camouflage: true,
      raidDraw: 1,
    },
  },
  zis_30: { combatAbilities: { camouflage: true } },

  // ===== Бронеавтомобили США: разведка, сопровождение и контрбатарея =====
  m2_scout_car: { onPlayEffects: { hqProtection: 1 } },
  m3_scout_car: { combatAbilities: { raidDraw: 1 } },
  m3a1_scout_car: { combatAbilities: { tankDefenseAura: 1 } },
  t13_armored_car: { onPlayEffects: { suppressEnemyIndirect: true } },

  // ===== САУ ===== (база: СУ-18, TKD) — остальные занимают огневую позицию;
  // Bison I бьет тем сильнее, чем дальше ушел от своей клетки спавна к штабу врага («Огневой вал»)
  su_5_2: { combatAbilities: { cornerBonus: { attack: 1 } } },
  wespe: { combatAbilities: { cornerBonus: { attack: 1 } } },
  su_122: { combatAbilities: { cornerBonus: { hp: 2 } } },
  sig_33_pzi: { combatAbilities: { hqProximityBonus: { maxBonus: 1 } } },
  su14: {
    onPlayEffects: { deployDamage: { amount: 1, scope: "classes", classes: ["light"] } },
  },
  t18_hmc: {
    combatAbilities: {
      cornerBonus: { hp: 2 },
      spawnDamageReduction: 1,
    },
  },
  t19_hmc: {
    combatAbilities: { cornerBonus: { attack: 1 } },
    onPlayEffects: { deployDamage: { amount: 1, scope: "random" } },
  },

  // ===== Миссия-трейлер (Курская дуга) =====
  // СУ-152 «Зверобой»: при выходе накрывает тяжелые цели и ПТ-САУ.
  su_152: {
    onPlayEffects: {
      deployDamage: { amount: 2, scope: "classes", classes: ["heavy", "td"] },
    },
  },
  // Ferdinand: непробиваемый лоб, фланг/тыл уязвимы.
  ferdinand: { combatAbilities: { frontalArmor: { amount: 2 } } },
};

for (const [cardId, override] of Object.entries(UNIT_ABILITY_OVERRIDES)) {
  const card = cards.find((item) => item.id === cardId);

  if (!card) continue;

  if (override.combatAbilities) {
    card.combatAbilities = { ...card.combatAbilities, ...override.combatAbilities };
  }

  if (override.onPlayEffects) {
    card.onPlayEffects = { ...card.onPlayEffects, ...override.onPlayEffects };
  }

  if (override.costModifiers) {
    card.costModifiers = override.costModifiers;
  }
}

const CARD_ID_ALIASES: Record<string, string> = {
  ba_10: "t-12",
  zis_5_supply: "amo_f15",
  gaz_aa_ammo: "ford_aa_ammo",
};

export function normalizeCardId(cardId: string): string | null {
  const resolvedCardId = CARD_ID_ALIASES[cardId] ?? cardId;
  const cardExists = cards.some((item) => item.id === resolvedCardId);

  return cardExists ? resolvedCardId : null;
}

export function getCardOrNull(cardId: string): TankCard | null {
  const resolvedCardId = normalizeCardId(cardId);

  if (!resolvedCardId) return null;

  return cards.find((item) => item.id === resolvedCardId) ?? null;
}

export function getCard(cardId: string): TankCard {
  const card = getCardOrNull(cardId);

  if (!card) {
    throw new Error(`Card not found: ${cardId}`);
  }

  return card;
}

