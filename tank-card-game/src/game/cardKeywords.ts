...
function getAbilityKeywords(card: TankCard): CardKeyword[] {
  ...
  if (draw > 0) {
    keywords.push({
      id: "deploy-draw",
      title: "РАЗМЕЩЕНИЕ",
      body: `Эффект «Размещение» срабатывает, когда вы выставляете этот отряд на поле боя: вы добираете ${draw === 1 ? "карту" : `${draw} карты`}.`,
    });
  }
  ...
  if (hqProtection > 0) {
    keywords.push({
      id: "deploy-hq",
      title: "РАЗМЕЩЕНИЕ",
      body: `Эффект «Размещение» срабатывает при выходе на поле боя: ваш штаб получает +${hqProtection} к здоровью.`,
    });
  }
