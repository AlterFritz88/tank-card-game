...
const DECK_CARD_IDS: Record<string, string[]> = {
  ...
  // Миссия 1 (боевое крещение): облегчена — вместо массы поддержки лёгкие
  // Panzer I A/B (реальные машины 4-й тд под Сталино), поддержки оставлено
  // немного, мотоциклов BMW R75 стало больше.
  ...
  // Миссия 2: масса средних Pz III/IV под Мценском — облегчена, часть ПТО и
  // поддержки заменена лёгкими Panzer I A/B, добавлены мотоциклы BMW R75.
  ...
  // Миссия 3: танковый корпус Гудериана — облегчён, часть ПТО и снабжения
  // заменена лёгкими Panzer I A/B, добавлены мотоциклы BMW R75.
  ...
  // Миссия 4: моторизованный авангард — облегчён. Прототипы Leichttraktor
  // (не воевали) заменены реальными для 1941 г. лёгкими Panzer I A/B; часть
  // поддержки убрана, мотоциклов BMW R75 стало больше — как и подобает дозору.
  ...
  // Миссия 5: 10-я тд штурмует деревню — облегчена, тяжёлая артиллерия и
  // пушки прорежены, вместо них лёгкие Panzer I A/B и мотоциклы BMW R75.
  ...
  // Миссия 7: штурм позиций Панфилова — облегчён, тяжёлая артиллерия и
  // снабжение прорежены, добавлены лёгкие Panzer I A/B и мотоциклы BMW R75.
  ...
  // Миссия 8: элита «Großdeutschland» — облегчена, часть ПТО, артиллерии и
  // разведтехники заменена лёгкими Panzer I A/B и мотоциклами BMW R75.
  ...
  // Миссия 9: 4-я тд, обескровленная зимой — ещё слабее: часть ПТО и
  // поддержки заменена лёгкими Panzer I A/B и мотоциклами BMW R75.
  ...
  // Миссия 10: заслон — облегчён, плотность ПТО и гаубиц снижена, вместо них
  // лёгкие Panzer I A/B и мотоциклы BMW R75; костяк StuG/PzJäger сохранён.
  ...
  // ============================================================
  // Кампания «Одинокий КВ» (Расейняй, июнь 1941) — колоды игрока
  // ============================================================

  // Миссии 1–2 (2-я танковая дивизия): тяжёлые КВ на острие, БТ и Т-26 в
  // разведке, дивизионная поддержка. Т-34 в 3-м мехкорпусе не было.
  soviet_2nd_td_campaign: [
    "kv1",
    "kv1",
    "kv1",
    "kv1_1940",
  ...
  // Миссия 5 («Экипаж КВ»): танк уже стоит на дороге (playerBoardUnits),
  // в колоде — отставшие машины дивизии и скудное снабжение.
  kv_crew_campaign: [
    "kv1",
    "kv1",
    "kv1_1940",
  ...
  // Миссии 3-4: тот же экипаж КВ, но вместо АТ-1 добавлен бронеавтомобиль БА-10.
  kv_crew_ba10_campaign: [
    "kv1",
    "kv1",
    "kv1_1940",
    "t26_1938",
  ...
  // База штаба «Экипаж КВ» для свободной игры после открытия (расширяется до 40).
  kv_crew_default: [
    "kv1",
    "kv1_1940",
    "kv2",
  ...
  // ============================================================
  // Кампания «Одинокий КВ» — колоды противника
  // ============================================================

  // Миссия 1: передовой разведотряд XXXXI корпуса — броневики, мотоциклы,
  // лёгкие танки. Лёгкая вводная миссия.
  first_panzer_border_campaign: [
    "sdkfz_221",
    "sdkfz_221",
    "sdkfz_222",
    "sdkfz_222",
  ...
  // Миссия 2: боевая группа Зекендорфа у Скаудвиле — масса Pz 35(t) и Pak 36,
  // чьи снаряды отскакивают от КВ (у игрока heavyArmorReduction штаба).
  german_6_panzer_campaign: [
    "panzer_35t",
    "panzer_35t",
    "panzer_35t",
    "panzer_35t",
  ...
  // Миссия 3: колонна снабжения группы Рауса — грузовики и штабные машины под
  // слабым охранением; батарея Pak 38, которую КВ расстрелял в упор.
  raus_supply_campaign: [
    "krupp_l3h163",
    "krupp_l3h163",
    "mercedes_g3a",
    "adler_type_10_n",
  ...
  // Миссия 4: ночь сапёров — пехотные орудия, sIG 33 и подрывные группы на
  // бронетранспортёрах пытаются подобраться к неподвижному КВ.
  raus_pioneers_campaign: [
    "leig_18",
    "leig_18",
    "sig_33_pzi",
    "sig_33_pzi",
  ...
  // Миссия 5: финал — Pz 35(t) кружат и отвлекают, пока разворачиваются
  // зенитки 8,8 cm — единственное, что пробивает КВ.
  raus_flak88_campaign: [
    "flak_88",
    "flak_88",
    "flak_88",
    "panzer_35t",
